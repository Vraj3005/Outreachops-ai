...
def get_active_template(user_id: str, email_type: str) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
        res = (
...
def create_prompt_template(template_in: PromptTemplateCreate) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
...
def update_prompt_template(
    template_id: str, template_in: PromptTemplateUpdate
) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
