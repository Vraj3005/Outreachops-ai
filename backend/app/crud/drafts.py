...
def check_existing_draft(
    lead_id: str, email_type: str, include_sent: bool = False
) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
...
def create_draft(draft_in: EmailDraftCreate) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
...
def update_draft(draft_id: str, draft_in: EmailDraftUpdate) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
...
def approve_draft(draft_id: str) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
        res = (
...
def reject_draft(draft_id: str) -> dict[str, Any] | None:
    if not supabase:
        return None
    try:
        res = (
