...
class AnalyticsService:
    ...
    def get_funnel_metrics(
        cls, user_id: str, campaign_id: str = None
    ) -> dict[str, int]:
        """
        ...
        try:
            ...
            if campaign_id:
                ...
                leads_q = (
                    leads_q.in_("id", lead_ids)
                    if lead_ids
                    else leads_q.eq("id", "none")
                )
            ...
            researched = sum(
                1
                for l in (res_res.data or [])
                if l.get("custom_fields") or l.get("tags")
            )
            ...
            # 4. Approved Drafts
            approved = sum(
                1 for d in drafts_data if d["status"] in ["approved", "sent"]
            )
            ...
            # 7. Replied
            replied = sum(
                1 for e in events_data if e["event_type"] in ["reply", "replied"]
            )

            # 8. Positive Replied
            positive = sum(
                1 for e in events_data if e["event_type"] == "positive_reply"
            )
        ...
        try:
            ...
            for v in variants:
                ...
                sends = sum(
                    1
                    for e in events
                    if e.get("variant_name") == v_name and e["event_type"] == "sent"
                )
                ...
                replies = sum(
                    1
                    for e in events
                    if e.get("variant_name") == v_name
                    and e["event_type"] in ["reply", "replied"]
                )
                ...
                report_variants.append(
                    {
                        "id": v["id"],
                        "name": v_name,
                        "prompt_template_version_id": v.get(
