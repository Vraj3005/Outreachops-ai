...
SPAM_WORDS = [
    "buy now", "click here", "guarantee", "risk-free", "double your",
    "100% free", "promise", "urgent", "credit card", "act fast",
    "limited time", "make money", "passive income", "earn cash",
    "special promotion", "unsolicited"
]
...
class EmailQualityService:
    ...
    def clean_and_normalize(self, subject: str, body: str) -> Dict[str, str]:
        """
        ...
        for line in lines:
            ...
            if normalized.startswith("SUBJECT:") or normalized.startswith("SUBJECT LINE:"):
                subject_leaked = True
                # Skip this line
                continue
            # Also check if it's the duplicate text of the subject line
            if subject and line.strip().strip('"').strip("'") == subject.strip().strip('"').strip("'"):
                subject_leaked = True
                continue
        ...
        signoffs = ["regards", "best regards", "thanks", "best", "sincerely", "cheers", "respectfully"]
        ...
        if found_signoff_index != -1:
            ...
            if name_idx != -1 and name_idx > len(cleaned_body) - 150:
                ...
                if agency.lower() not in body_lower and sender_name.lower() not in body_lower:
    ...
    def evaluate_draft(self, subject: str, body: str, email_type: str, lead: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        # 1. Clean first
        cleaned = self.clean_and_normalize(subject, body)
        ...
        if subject_leaked_flag or "subject:" in body.lower() or "subject line:" in body.lower():
        ...
        if ideal_min <= words <= ideal_max:
            ...
            diff_from_bounds = (ideal_min - words) if words < ideal_min else (words - ideal_max)
        ...
        sender_name = settings.YOUR_NAME.lower()
        agency = settings.YOUR_AGENCY_NAME.lower()
        if sender_name not in cleaned_body.lower() and agency not in cleaned_body.lower():
        ...
        for line in reversed(content_lines):
            ...
            if settings.YOUR_NAME.lower() in line.lower() or settings.YOUR_WEBSITE.lower() in line.lower() or "best" in line.lower() or "regards" in line.lower():
                continue
            if "?" in line or "call" in line.lower() or "meet" in line.lower() or "chat" in line.lower() or "review" in line.lower() or "mockup" in line.lower():
                cta_text = line
                break
        ...
        if not cta_text and len(content_lines) >= 2:
            ...
            non_sig_lines = [l for l in content_lines if not any(k in l.lower() for k in ["best,", "regards,", "thanks,", settings.YOUR_NAME.lower(), settings.YOUR_AGENCY_NAME.lower()])]
            if non_sig_lines:
        ...
        if website and (website in cleaned_body.lower() or website.split(".")[0] in cleaned_body.lower()):
            personalization_score += 3.0
            
        # Check if pain points or erp approach is mentioned in some way
        pain_points = (lead.get("website_pain_points") or lead.get("erp_approach") or "").lower()
        if pain_points:
            # check if keywords or sub-phrases from pain points are used
            keywords = [kw.strip() for kw in re.split(r'[,.\s]+', pain_points) if len(kw.strip()) > 4]
            matches = 0
        ...
        stop_words = {"the", "a", "and", "or", "of", "in", "to", "for", "is", "that", "on", "with", "this", "our", "your", "we", "you", "i", "it", "are", "be", "at", "as", "by", "an"}
        ...
        raw_quality = (length_score + personalization_score + clarity_score + (10.0 - spam_risk_score) + repetition_score) / 5.0
