...
class GmailSyncService:
    ...
    def sync_user_replies(cls, user_id: str):
        ...
        conn_res = supabase.table("integration_connections")\
            .select("*")\
            .eq("user_id", user_id)\
            .eq("provider", "gmail")\
            .execute()
        ...
        if last_history_id:
            try:
                logger.info(f"Initiating incremental Gmail history sync from history ID: {last_history_id}")
                history_res = gmail_client.users().history().list(
                    userId="me", 
                ...
                if e.resp.status in [400, 404]:
                    logger.warning(f"History ID {last_history_id} expired. Falling back to Inbox scan.")
        ...
        cls._process_messages_batch(user_id, gmail_client, user_email, messages_to_process)
    ...
    def _fetch_inbox_fallback(cls, gmail_client) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            res = gmail_client.users().messages().list(userId="me", q="is:inbox", maxResults=50).execute()
    ...
    @classmethod
    def _process_messages_batch(cls, user_id: str, gmail_client, user_email: str, messages: List[Dict[str, Any]]):
        ...
        for msg_summary in messages:
            ...
            try:
                exist_check = supabase.table("reply_events").select("id").eq("gmail_message_id", msg_id).execute()
                if exist_check.data:
                    continue
            except Exception:
                pass
            ...
            try:
                msg = gmail_client.users().messages().get(userId="me", id=msg_id, format="full").execute()
            except Exception as e:
            ...
            if not lead_id or not campaign_id:
                logger.debug(f"Message thread {thread_id} does not map to any active outreach lead. Skipped.")
                continue
    ...
    @classmethod
    def _resolve_thread_mapping(cls, thread_id: str, clean_sender: str) -> tuple[str | None, str | None]:
        """Resolves lead and campaign associations by matching the Gmail thread ID."""
        try:
            # Query send events by thread id
            res = supabase.table("send_events")\
            ...
            lead_res = supabase.table("leads").select("id").eq("contact_email", clean_sender).execute()
            if lead_res.data:
    ...
    def _mock_demo_sync(cls, user_id: str):
        ...
        logger.info(f"[Demo Mode] Simulating inbound reply scan tick for user {user_id}")
        ...
        try:
            ...
            rep_check = supabase.table("reply_events")\
                .select("id")\
                .eq("lead_id", lead_id)\
            ...
            lead = supabase.table("leads").select("contact_email").eq("id", lead_id).execute().data[0]
            
            # Generate simulated reply
            import random
            sim_types = [
                ("Interested in scheduling a meeting. Call me next week.", "positive/interested"),
                ("Please unsubscribe me from this list immediately.", "unsubscribe"),
                ("This is an out of office reply. I am on vacation until Monday.", "out of office")
            ]
