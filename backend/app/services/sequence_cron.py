        ...
        for cl in waiting_leads.data or []:
            if cl["campaign_id"] in active_campaign_ids:
                try:
                    ...
                    logger.info(
                        f"Transitioned lead {cl['lead_id']} from waiting to awaiting_generation"
                    )
                except Exception as e:
        ...
        for cl in gen_leads.data or []:
            if cl["campaign_id"] in active_campaign_ids:
                try:
                    ...
                    logger.info(
                        f"Auto-generated draft for lead {cl['lead_id']} current step {cl['current_sequence_step']}"
                    )
                except Exception as e:
            ...
            if cl["campaign_id"] in active_campaign_ids:
                ...
                if drafts_res.data:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Dispatched scheduled email for lead {cl['lead_id']} (Draft: {draft_id})"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to dispatch scheduled email for lead {cl['id']}: {e}"
                        )
                else:
