...
class SequenceService:
    ...
    def get_or_create_default_sequence(cls, campaign_id: str, user_id: str) -> str:
        ...
        supabase.table("campaigns").update({"sequence_id": seq_id}).eq("id", campaign_id).execute()
        logger.info(f"Associated default 3-step sequence {seq_id} with campaign {campaign_id}")
        return seq_id
    ...
    def calculate_next_send_time(
        ...
        if isinstance(tz, datetime.timezone):
            ...
            try:
                ...
                local_dt = from_utc_dt.astimezone(datetime.timezone.utc)
    ...
    @classmethod
    def enroll_lead(cls, campaign_id: str, lead_id: str, user_id: str) -> Dict[str, Any]:
        """
        ...
        existing = supabase.table("campaign_leads").select("*").eq("campaign_id", campaign_id).eq("lead_id", lead_id).execute()
        if existing.data:
    ...
    def generate_draft_for_current_step(cls, campaign_lead_id: str) -> Dict[str, Any]:
        ...
        """
        cl_res = supabase.table("campaign_leads").select("*").eq("id", campaign_lead_id).execute()
        if not cl_res.data:
        ...
        set_res = supabase.table("owner_settings").select("*").eq("user_id", campaign["user_id"]).execute()
        ...
        ai_res = gemini.generate_email_content(instruction_payload, user_id=campaign["user_id"])
        
        subject = ai_res.get("subject") or prev_subject or f"Follow-up regarding {campaign.get('name')}"
        body = ai_res.get("body") or ""

        # Quality evaluation
        quality_checker = EmailQualityService()
        eval_res = quality_checker.evaluate_draft(subject, body, campaign.get("campaign_type") or "generic", lead)
        ...
        supabase.table("campaign_leads").update({
            "status": lead_status,
            "next_step_scheduled_at": scheduled_time_str
        }).eq("id", campaign_lead_id).execute()
    ...
    def approve_lead_draft(cls, campaign_lead_id: str, draft_id: str) -> Dict[str, Any]:
        ...
        supabase.table("email_drafts").update({"status": "approved"}).eq("id", draft_id).execute()
    ...
    @classmethod
    def dispatch_scheduled_email(cls, campaign_lead_id: str, draft_id: str) -> Dict[str, Any]:
        """
        ...
        stop_needed, stop_reason = cls.evaluate_stop_conditions(cl["campaign_id"], cl["lead_id"])
        if stop_needed:
            supabase.table("campaign_leads").update({
                "status": "stopped",
                "stopped_reason": stop_reason,
        ...
        res = gmail_service.send_approved_email(draft_id=draft_id, user_id=cl["user_id"])
    ...
    def transition_sent(cls, campaign_lead_id: str) -> Dict[str, Any]:
        ...
        camp_res = supabase.table("campaigns").select("*").eq("id", cl["campaign_id"]).execute()
        campaign = camp_res.data[0]
    ...
    @classmethod
    def evaluate_stop_conditions(cls, campaign_id: str, lead_id: str) -> tuple[bool, Optional[str]]:
        """
        ...
        if email:
            dnc_check = supabase.table("do_not_contact").select("id").eq("email", email.strip().lower()).execute()
            if dnc_check.data:
        ...
        events_res = supabase.table("send_events").select("*").eq("campaign_id", campaign_id).eq("lead_id", lead_id).execute()
    ...
    def recalculate_paused_campaign_leads(cls, campaign_id: str) -> None:
        ...
        """
        leads_res = supabase.table("campaign_leads").select("*").eq("campaign_id", campaign_id).execute()
        for cl in (leads_res.data or []):
            if cl.get("status") in ["awaiting_generation", "awaiting_approval", "scheduled", "stopped"] and cl.get("stopped_reason") == "Campaign paused":
