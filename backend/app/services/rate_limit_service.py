...
class RateLimitService:
    ...
    """
    def __init__(self):
        self.daily_send_limit = int(os.getenv("DAILY_SEND_LIMIT", "50")) if "os" in globals() else 50
    ...
    def check_daily_limit(self, user_id: str, cap: int = None) -> bool:
        ...
        try:
            res = supabase.table("send_logs").select("id", count="exact") \
                .eq("user_id", user_id) \
                .eq("status", "sent") \
            ...
            logger.info(f"User {user_id} has sent {sent_today}/{max_emails} emails today.")
    ...
    def check_double_email_limit(self, lead_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check double outreach limits for lead {lead_id}: {e}")
            return True
