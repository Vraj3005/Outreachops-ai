...
class ReplyClassificationService:
    ...
    def classify_and_process(
        ...
        import re
        body_excerpt = re.sub(r'\b\d{3}[-.]?\d{3}[-.]?\d{4}\b', "[PHONE-REDACTED]", body_excerpt)
        body_excerpt = re.sub(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', "[EMAIL-REDACTED]", body_excerpt)

        # 2. Apply Deterministic Rules First
        category, confidence, rule_used, explanation = cls._apply_rules(sender, subject, body)

        # 3. Optional AI Classification Fallback
        if category is None or confidence < 0.8:
            ai_cat, ai_conf, ai_explain = cls._classify_with_ai(user_id, subject, body_excerpt)
            if ai_cat:
                category = ai_cat
                confidence = ai_conf
                rule_used = "gemini-ai"
                ...
                explanation = explanation or "Categorized using keyword pattern matches."
        ...
        cls._emit_analytics_event(user_id, campaign_id, lead_id, category, gmail_message_id)
        ...
        if category in ["positive/interested", "meeting request", "unclear", "referral"]:
            cls._create_suggested_reply_draft(user_id, campaign_id, lead_id, subject, body_excerpt)

        return reply_record

    @classmethod
    def _apply_rules(cls, sender: str, subject: str, body: str) -> Tuple[str, float, str, str]:
        ...
        bounce_sub = ["undeliverable", "returned mail", "delivery failure", "delivery status notification", "failure notice"]
        if any(b in sender_lower for b in bouncers) or any(s in sub_lower for s in bounce_sub):
            return "bounce/delivery failure", 1.0, "deterministic-rules", "Inbound sender domain or subject matches automated mail daemon failure signature."

        # B. Out of office (OOO)
        ooo_triggers = ["out of office", "auto-reply", "autoreply", "vacation", "away from my", "ooo"]
        if any(t in sub_lower for t in ooo_triggers) or any(t in body_lower for t in ooo_triggers):
            return "out of office", 1.0, "deterministic-rules", "Subject/body matches automated out-of-office autoreply templates."

        # C. Unsubscribe request
        unsub_triggers = ["unsubscribe", "remove me", "stop writing", "do not email", "take me off"]
        if any(t in sub_lower for t in unsub_triggers) or any(t in body_lower for t in unsub_triggers):
            return "unsubscribe", 0.9, "deterministic-rules", "Contains explicit opt-out keywords like unsubscribe/remove."

        return None, 0.0, None, None

    @classmethod
    def _classify_with_ai(cls, user_id: str, subject: str, body_excerpt: str) -> Tuple[str, float, str]:
        ...
        try:
            ...
            model_name = gemini.model_list[0] if gemini.model_list else "gemini-2.5-flash-lite"
            ...
            return matched_cat or "unclear", float(data.get("confidence", 0.7)), data.get("explanation", "")
        except Exception as e:
            logger.error(f"Gemini reply classification failed: {e}")
            return None, 0.0, None

    @classmethod
    def _apply_sequence_stopping(cls, user_id: str, campaign_id: str, lead_id: str, category: str):
        """Immediately halts pending steps based on category type or campaign OOO options."""
        now_str = datetime.datetime.now(datetime.UTC).isoformat()
        
        # Determine behavior
        should_stop = category in ["unsubscribe", "bounce/delivery failure", "not interested", "wrong person", "positive/interested", "meeting request", "referral", "not now"]
        
        if category == "out of office":
            # Fetch campaign OOO behavior config
            try:
                camp_res = supabase.table("campaigns").select("ooo_behavior").eq("id", campaign_id).execute()
        ...
        if should_stop:
            new_status = "replied" if category in ["positive/interested", "meeting request", "referral", "unclear"] else "stopped"
            ...
            }).eq("campaign_id", campaign_id).eq("lead_id", lead_id).in_("status", ["pending", "retry"]).execute()

            logger.info(f"Halted sequence steps for lead {lead_id} under category {category}")

    @classmethod
    def _emit_analytics_event(cls, user_id: str, campaign_id: str, lead_id: str, category: str, message_id: str):
    ...
    @classmethod
    def _create_suggested_reply_draft(cls, user_id: str, campaign_id: str, lead_id: str, orig_subject: str, orig_body: str):
        ...
        try:
            ...
            logger.info(f"Generated suggested reply draft {draft_id} for lead {lead_id}")
        except Exception as e:
