...
class PreviewRequest(BaseModel):
    ...
    field_mapping: Dict[str, str] = Field(..., description="Mapping of source headers to target fields")

class CommitRequest(BaseModel):
    fingerprint: str = Field(..., description="Import payload cache fingerprint")
    field_mapping: Dict[str, str] = Field(..., description="Mapping of source headers to target fields")
    source_name: str = Field(..., description="Import source display name")
    source_type: str = Field(..., description="Import source type: 'csv', 'xlsx', 'google_sheets'")
    preset_name: Optional[str] = Field(None, description="Optional name to save mapping preset as")
    ...
    if file:
        ...
        if settings.DEMO_MODE:
            ...
            mock_sheet_rows = [
                ...
                ["2", "apex-roofing-mock.com", "Apex Roofing Solutions", "Slow forms", "centralized scheduling", "contact@apex-roofing-mock.com"],
                ["3", "beacon-masonry-mock.com", "Beacon Masonry Inc", "No CTA", "job-costing ledger", "estimates@beacon-masonry-mock.com"],
                ["4", "summit-hvac-mock.com", "Summit HVAC", "Broken booking tool", "dispatch board", "service@summit-hvac-mock.com"]
            ]
        ...
        try:
            ...
            sheet = client.open_by_key(sheet_id) if len(sheet_id) > 20 else client.open(sheet_url)
...
async def preview_import_validation(payload: PreviewRequest, owner: dict = Depends(require_owner)):
    ...
    try:
        ...
        res["auto_suggestions"] = import_service.auto_suggest_mappings(
            headers=res["preview_items"][0]["original_cells"] if res["preview_items"] else []
        )
        return res
    except Exception as e:
...
async def commit_import_leads(payload: CommitRequest, owner: dict = Depends(require_owner)):
...
async def download_errors_csv(fingerprint: str = Query(..., description="Payload cache fingerprint")):
    ...
    try:
        ...
        return StreamingResponse(
            iter([csv_content]),
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=import_validation_errors_{fingerprint}.csv"}
        )
    except Exception as e:
...
async def list_mapping_presets(owner: dict = Depends(require_owner)):
    ...
    if supabase:
        try:
            res = supabase.table("import_mappings").select("*").eq("user_id", owner["id"]).execute()
            if res.data:
...
async def save_mapping_preset(payload: MappingPresetSaveRequest, owner: dict = Depends(require_owner)):
    """Saves a new mapping preset configuration."""
    if not supabase:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database offline")
    ...
    try:
        ...
        if not res.data:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save mapping preset")
        return res.data[0]
    except Exception as e:
