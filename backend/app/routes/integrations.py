...
class SheetsConfigRequest(BaseModel):
    service_account_json: str = Field(..., description="Service account credentials JSON string")

class GeminiConfigRequest(BaseModel):
    api_key: str = Field(..., description="Gemini API Key")
    allowed_model: str = Field("gemini-2.5-flash-lite", description="Allowed Gemini Model")
...
async def get_gmail_status(owner: dict = Depends(require_owner)):
    ...
    try:
        ...
        if res["status"] == "connected":
            ...
            db_res = supabase.table("integration_connections").select("encrypted_credentials").eq("user_id", user_id).eq("provider", "gmail").execute()
            if db_res.data:
                try:
...
async def connect_gmail_account(owner: dict = Depends(require_owner)):
    ...
    try:
        ...
        db_res = supabase.table("integration_connections").select("*").eq("user_id", user_id).eq("provider", "gmail").execute()
        if db_res.data:
            dec = decrypt_val(db_res.data[0]["encrypted_credentials"])
            info = json.loads(dec)
            info["email"] = email_addr
            encrypted = encrypt_val(json.dumps(info))
            supabase.table("integration_connections").update({"encrypted_credentials": encrypted}).eq("id", db_res.data[0]["id"]).execute()
    except Exception as e:
...
async def get_sheets_status(owner: dict = Depends(require_owner)):
    ...
    if settings.DEMO_MODE:
        return {
            ...
            "available_spreadsheets": [settings.GOOGLE_SHEET_NAME, "Demo Campaign Sheets", "Sales Leads 2026"]
        }
...
async def configure_google_sheets(payload: SheetsConfigRequest, owner: dict = Depends(require_owner)):
    """
    ...
    try:
        ...
        res = supabase.table("integration_connections").select("id").eq("user_id", user_id).eq("provider", "google_sheets").execute()
        if res.data:
            supabase.table("integration_connections").update(db_payload).eq("id", res.data[0]["id"]).execute()
        else:
            import uuid
            db_payload["id"] = str(uuid.uuid4())
            supabase.table("integration_connections").insert(db_payload).execute()
            
        return {"status": "success", "message": "Google Sheets service account configured successfully."}
    except Exception as e:
...
async def disconnect_google_sheets(owner: dict = Depends(require_owner)):
    ...
    if settings.DEMO_MODE:
        return {"status": "success", "message": "Demo mode connection cleared."}
    try:
        supabase.table("integration_connections").delete().eq("user_id", user_id).eq("provider", "google_sheets").execute()
        return {"status": "success", "message": "Google Sheets connection disconnected."}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def configure_gemini(payload: GeminiConfigRequest, owner: dict = Depends(require_owner)):
    """
    ...
    try:
        ...
        # Check existing connection record
        res = supabase.table("integration_connections").select("id").eq("user_id", user_id).eq("provider", "gemini").execute()
        if res.data:
            supabase.table("integration_connections").update(db_payload).eq("id", res.data[0]["id"]).execute()
        else:
            import uuid
        ...
        return {"status": "success", "message": "Gemini connection settings saved successfully."}
    except Exception as e:
...
async def disconnect_gemini(owner: dict = Depends(require_owner)):
    ...
    user_id = owner["id"]
    try:
        supabase.table("integration_connections").delete().eq("user_id", user_id).eq("provider", "gemini").execute()
