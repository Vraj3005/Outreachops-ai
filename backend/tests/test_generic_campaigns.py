...
def test_campaign_migration_logic():
    ...
    cursor.execute(
        "INSERT INTO campaigns VALUES ('1', 'erp', 'ERP consulting campaign', '')"
    )
    cursor.execute(
        "INSERT INTO campaigns VALUES ('2', 'website', 'Web design campaign', NULL)"
    )
    cursor.execute(
        "INSERT INTO campaigns VALUES ('3', 'mixed', 'General agency campaign', 'Custom Objective')"
    )
    cursor.execute(
        "INSERT INTO campaigns VALUES ('4', 'generic', 'Already generic campaign', 'Custom Objective 2')"
    )
    conn.commit()
    ...
    assert rows[0] == (
        "1",
        "generic",
        "Introduce an ERP consulting service and propose schedule systems",
    )
    assert rows[1] == (
        "2",
        "generic",
...
def test_campaign_cloning_and_snapshots(mocker):
    ...
    mock_camp_query.select.return_value.eq.return_value.eq.return_value.execute.return_value = mocker.Mock(
        data=[original_campaign]
    )

    # Mock sequence and steps select
    mock_seq_query = mocker.Mock()
    mock_seq_query.select.return_value.eq.return_value.execute.return_value = (
        mocker.Mock(data=[{"id": "original-seq-456", "name": "Main Sequence"}])
    )

    mock_steps_query = mocker.Mock()
    mock_steps_query.select.return_value.eq.return_value.execute.return_value = (
        mocker.Mock(
            data=[
                {
                    "id": "step-1",
                    "sequence_id": "original-seq-456",
                    "step_number": 1,
                    "subject_instruction": "Intro",
                    "body_instruction": "Pitch",
                }
            ]
        )
    )
    ...
    mock_camp_query.insert.return_value.execute.return_value = mocker.Mock(
        data=[inserted_cloned_campaign]
    )
...
def test_campaign_presets_creation(mocker):
    ...
    mock_supabase.table.return_value.insert.return_value.execute.return_value = (
        mocker.Mock(data=[inserted_preset])
    )
    mocker.patch("app.routes.campaigns.supabase", mock_supabase)
    mocker.patch("app.crud.campaigns.supabase", mock_supabase)
...
def test_campaign_preview_generation(mocker):
    ...
    mock_supabase = mocker.MagicMock()
    mock_supabase.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.return_value = mocker.Mock(
        data=[
            {
                "company_name": "Delta Roofing",
                "first_name": "David",
                "job_title": "General Manager",
                "website": "https://deltaroofing.com",
            }
        ]
    )
    mocker.patch("app.routes.campaigns.supabase", mock_supabase)
