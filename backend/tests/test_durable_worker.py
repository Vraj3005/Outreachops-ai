...
def test_guardrails_unsubscribed_tag_block(mocker):
    ...
    def select_side_effect(table):
        ...
        if table == "do_not_contact":
            mock_tbl.select.return_value.eq.return_value.eq.return_value.execute.return_value = mocker.Mock(
                data=[]
            )
        else:
            mock_tbl.select.return_value.eq.return_value.eq.return_value.execute.return_value = mocker.Mock(
...
def test_worker_transient_retry_handling(mocker):
    ...
    DurableSendingWorker._mark_failed(
        job, "Rate Limit Exceeded (429)", is_transient=True
    )
...
def test_idempotency_duplicate_dispatch_check(mocker):
    ...
    mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.eq.return_value.eq.return_value.execute.return_value = mocker.Mock(
        data=[{"id": "event-1"}]
    )
    mocker.patch("app.services.durable_sending_worker.supabase", mock_supabase)
