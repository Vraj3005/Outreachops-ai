...
def test_ooo_config_handling(mocker):
    ...
    mock_supabase.table.return_value.select.return_value.eq.return_value.execute.return_value = mocker.Mock(data=[{"ooo_behavior": "pause"}])
    mocker.patch("app.services.reply_classification_service.supabase", mock_supabase)
...
def test_own_message_ignored(mocker):
    ...
    mock_gmail.users.return_value.getProfile.return_value.execute.return_value = {"emailAddress": "user@gmail.com"}
    ...
    GmailSyncService._process_messages_batch("user-1", mock_gmail, "user@gmail.com", messages)
...
def test_history_expiration_fallback(mocker):
    ...
    mock_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.execute.return_value = mocker.Mock(data=[{
        "user_id": "user-1",
        "last_history_id": "9999"
    }])
    mocker.patch("app.services.gmail_sync_service.supabase", mock_supabase)

    mock_gmail = mocker.MagicMock()
    mocker.patch("app.services.gmail_service.GmailService._get_gmail_client", return_value=mock_gmail)
    mocker.patch("app.services.gmail_service.GmailService.check_connection_status", return_value={"status": "connected"})

    # Raise HttpError on history list
    resp = mocker.Mock(status=404)
    mock_gmail.users.return_value.history.return_value.list.return_value.execute.side_effect = HttpError(resp=resp, content=b"History ID expired")
    ...
    mock_gmail.users.return_value.getProfile.return_value.execute.return_value = {"emailAddress": "user@gmail.com", "historyId": "10050"}
    ...
    mock_gmail.users.return_value.messages.return_value.list.assert_called_with(userId="me", q="is:inbox", maxResults=50)
