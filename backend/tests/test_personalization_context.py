...
def test_compile_context_no_research():
    ...
    assert any(
        "No recent public website research snapshot found" in w for w in warnings
    )
...
def test_detect_field_conflicts():
    ...
    assert any(
        "Email domain" in w and "mismatches website domain" in w for w in warnings_dom
    )
...
def test_custom_fields_and_exclusions():
    lead = {
        ...
        "custom_fields": {"tier": "enterprise", "secret_key": "12345-secret"},
    }
...
def test_unsupported_campaign_claims():
    ...
    warnings = res["missing_context_warnings"]
    assert any(
        "Prohibited Claim warning" in w and "first place in global ranking" in w
        for w in warnings
    )
