...
def test_clean_and_normalize(mocker):
    ...
    mocker.patch(
        "app.services.email_quality_service.settings.YOUR_AGENCY_NAME",
        "Pitbull Corporations",
    )
    mocker.patch(
        "app.services.email_quality_service.settings.YOUR_WEBSITE",
        "https://pitbullcorporations.com",
    )
    mocker.patch(
        "app.services.email_quality_service.settings.YOUR_PHONE", "+91-7801951876"
    )
    ...
    assert (
        "Outreach Proposal" not in res["body"].split("\n")[0]
    )  # first line shouldn't contain it
    assert (
        "Vraj | Pitbull Corporations | https://pitbullcorporations.com | +91-7801951876"
        in res["body"]
    )
...
def test_evaluate_draft_banned_phrases(mocker):
    ...
    assert (
        eval_res["scores"]["quality_score"] < 8.0
    )  # Quality score should be penalized for warnings
...
def test_evaluate_draft_length_and_personalization(mocker):
    ...
    assert (
        eval_res["scores"]["personalization_score"] > 5.0
    )  # apex.com and Apex are referenced
