...
def test_sequence_stop_conditions_check(mocker):
    ...
    def select_side_effect(table):
        ...
        if table == "leads":
            mock_tbl.select.return_value.eq.return_value.execute.return_value = (
                mocker.Mock(data=[mock_lead])
            )
        elif table == "do_not_contact":
            mock_tbl.select.return_value.eq.return_value.execute.return_value = (
                mocker.Mock(data=[{"id": "dnc-1"}])
            )
        else:
            mock_tbl.select.return_value.eq.return_value.execute.return_value = (
                mocker.Mock(data=[])
    ...
    def select_side_effect_tags(table):
        ...
        if table == "leads":
            mock_tbl.select.return_value.eq.return_value.execute.return_value = (
                mocker.Mock(data=[mock_lead_opt_out])
            )
        elif table == "do_not_contact":
            mock_tbl.select.return_value.eq.return_value.execute.return_value = (
                mocker.Mock(data=[])
...
def test_enroll_lead_state_machine_transition(mocker):
    ...
    mocker.patch.object(
        SequenceService, "get_or_create_default_sequence", return_value="seq-123"
    )
    ...
    assert (
        mock_supabase.table.return_value.insert.called
        or mock_supabase.table.return_value.update.called
    )
